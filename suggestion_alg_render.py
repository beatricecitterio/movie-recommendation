...
data_dir = './movielens_data_render'
os.makedirs(data_dir, exist_ok=True)

# Download the dataset using Kaggle API
os.system(f'kaggle datasets download -d ayushimishra2809/movielens-dataset -p {data_dir}')

# Extract the downloaded zip file
zip_path = f'{data_dir}/movielens-dataset.zip'
...
average_rating = ratings.groupby('movieId')['rating'].mean().reset_index()
average_rating.rename(columns={'rating': 'avg_rating'}, inplace=True)

average_rating = pd.merge(movies, average_rating, on='movieId')
...
def suggestion(new_rating: list, genre: str, n=5, number_of_suggestions=3):
    ...
    Based on the user's ratings, the function finds the most similar users in the dataframe, computes their average rating over the selected
    ...
    mustsee_suggestions = mustsee_suggestions.merge(average_rating[['movieId', 'avg_rating']], on='movieId')

    return (final_suggestions[['Formatted Title', 'avg_rating']].values.tolist()), (mustsee_suggestions[['Formatted Title', 'avg_rating']].values.tolist())